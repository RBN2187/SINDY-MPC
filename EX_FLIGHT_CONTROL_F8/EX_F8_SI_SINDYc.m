...
% --- Call the getTrainingData function to get measurements of the system ---
getTrainingData
u = u'; % Transpose the input signal

%% 2. SINDYc
% Parameters of the SINDYc algorithm
ModelName = 'SINDYc'; 
polyorder = 3;
usesine = 0;
...
% --- Call the trainSINDYc function to identify the system ---
trainSINDYc

% Comparison of the identified model with the true model
% Xi is the sparse regression coefficient matrix identified by SINDYc, Xi0 is the true model parameters
Xi0 = zeros(size(Xi));
...
disp('Element-wise difference between identified and true model parameters:')
Xi-Xi0

%% 3. PREDICTION OVER TRAINING PHASE
p.ahat = Xi(:,1:Nvar);
...
%% Plotting the Fit of the model to the training data
clear ph
...
%% Prediciton over validation phase and plotting
% Ground Truth from the validation data
tspanV   = [100:dt:200];
xA      = xv;
tA      = tv;

% Predicted values using the identified SINDYc model
[N,Ns] = size(xA);
...
% --- Call the VIZ_SI_Validation function to visualize the validation results ---
VIZ_SI_Validation
